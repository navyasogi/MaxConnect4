...
public class GameBoard {
    ...
    private maxconnect4.PARTCIPANT first_turn;
    private maxconnect4.GAME game_mode;

    public static final int COLUMNS = 7;
    public static final int ROWS = 6;
    public static final int MAX_PIECES = 42;

    /**
     * This constructor creates a GameBoard object using input file. It reads data from the input file
     * and will display exactly what has been read in from the input file. 
     * 
    ...
    public GameBoard(String inputFile) {
        ...
        for (int i = 0; i < ROWS; i++) {
            try {
                ...
                for (int j = 0; j < COLUMNS; j++) {
                    ...
                    // sanity check
                    if (!((this.playBoard[i][j] == 0) || (this.playBoard[i][j] == maxconnect4.FIRST) || (this.playBoard[i][j] == maxconnect4.SECOND))) {
        ...
        if (!((this.currentTurn == maxconnect4.FIRST) || (this.currentTurn == maxconnect4.SECOND))) {
    ...
    public void setPieceValue() {
        if ((this.currentTurn == maxconnect4.FIRST && first_turn == maxconnect4.PARTCIPANT.COMPUTER)
            || (this.currentTurn == maxconnect4.SECOND && first_turn == maxconnect4.PARTCIPANT.HUMAN)) {
            maxconnect4.COMPUTER_PLAYER = maxconnect4.FIRST;
            maxconnect4.HUMAN_PLAYER = maxconnect4.SECOND;
        } else {
            maxconnect4.HUMAN_PLAYER = maxconnect4.FIRST;
            maxconnect4.COMPUTER_PLAYER = maxconnect4.SECOND;
        }
        
        System.out.println("Human plays as : " + maxconnect4.HUMAN_PLAYER + " , Computer plays as : " + maxconnect4.COMPUTER_PLAYER);
    ...
    boolean isBoardFull() {
        return (this.getPieceCount() >= GameBoard.MAX_PIECES);
    }
    ...
    //set first turn
    public void setFirstTurn(maxconnect4.PARTCIPANT turn) {
        first_turn = turn;
        setPieceValue();
    }

    public maxconnect4.PARTCIPANT getFirstTurn() {
        return first_turn;
    }
    //set game mode (interactive or one-move)
    public void setGameMode(maxconnect4.GAME mode) {
        game_mode = mode;
    }

    public maxconnect4.GAME getGameMode() {
        return game_mode;
    }
